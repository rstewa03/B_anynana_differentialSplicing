...
Metadata <- read.csv("/cerberus/projects/racste/B_anynana/RNA_data/B_anynana_metadataForSupplement.csv")[,-1] %>% 
  arrange(Run) %>% 
  dplyr::select(Run, Season, Food.treatment, Family, Body.part) %>% 
  mutate(season_stress = paste0(Season, Food.treatment))

load(file = "02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/star_fc_B_anynana_genes_dgelist.Rdata")
y.genes.all <- y.genes.all[, Metadata$Run]

#### ABDOMEN: filterByExpr ####
DE.data <- list()
body.part <- "thorax"
...
pdf(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/edgeR_DE_modelFit_",body.part,".pdf"), 
    height = 8, width = 8)
par(mfrow = c(2, 2))
...
save(fit.genes.filt.SxF, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/fit.genes.",body.part,".SxF.Rdata"))
save(fit.genes.filt.SandF, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/fit.genes.",body.part,".SandF.Rdata"))

#### differential expression: filterByExpr ####
# load(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/fit.genes.",body.part,".SxF.Rdata"))
# load(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/fit.genes.",body.part,".SandF.Rdata"))
...
all.pvals <- rbind(dplyr::select(qlf.genes$interaction, GeneID, PValue, FDR, effect), 
...
qlf.genes$interaction$adjP.Value <- all.pvals$all.adjp[all.pvals$effect == "interaction"]
qlf.genes$season$adjP.Value <- all.pvals$all.adjp[all.pvals$effect == "season"]
qlf.genes$family$adjP.Value <- all.pvals$all.adjp[all.pvals$effect == "family"]

for (i in 1:3){
  write_tsv(qlf.genes[[i]], paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/edgeR_DE_qlfGenes_",body.part,"_", names(qlf.genes)[i],".tsv")) 
}
...
# adjusted p-value for differentially spliced genes
all.pvals.multiexon <- rbind(dplyr::select(qlf.genes.multiexon$interaction, GeneID, PValue, FDR, effect), 
...
qlf.genes.multiexon$interaction$adjP.Value <- all.pvals.multiexon$all.adjp[all.pvals.multiexon$effect == "interaction"]
qlf.genes.multiexon$season$adjP.Value <- all.pvals.multiexon$all.adjp[all.pvals.multiexon$effect == "season"]
qlf.genes.multiexon$family$adjP.Value <- all.pvals.multiexon$all.adjp[all.pvals.multiexon$effect == "family"]

family_logfc <- cbind(qlf.genes.multiexon$family[8:13], rep(0,DE.data$genes_multiexon))
qlf.genes.multiexon$family <- left_join(qlf.genes.multiexon$family,
                              cbind(GeneID = qlf.genes.multiexon$family$GeneID, 
                              data.frame(Means = rowMeans(as.matrix(family_logfc[,1:6]))),
                              data.frame(Ranges = rowMaxs(as.matrix(family_logfc))-rowMins(as.matrix(family_logfc)))))

for (i in 1:3){
  write_tsv(qlf.genes.multiexon[[i]], paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/edgeR_DE_qlfMultiexonGenes_",body.part,"_", names(qlf.genes.multiexon)[i],".tsv")) 
}
...
write_tsv(DE.data$DE_summary, paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/DE_summary_",body.part,".tsv"))
...
save(DE_joined, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/DE_joined_",body.part))

for (i in 1:3){
  write_tsv(DE_joined[[i]], paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/DE_joined_",body.part,"_", names(DE_joined)[i],".tsv")) 
}

save(DE.data, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/DE.data.", body.part))

#### Figures #### 

color2 <-c( "cornflowerblue", "cornflowerblue", "cornflowerblue")
color_transparent2 <- adjustcolor(color2, alpha.f = 0.75)
# Upset plots
pdf(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Overlap/edgeR_DE_upset_",body.part,".pdf"), 
    height = 4,
    width = 6)
...
# Euler venn diagrams
pdf(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Overlap/edgeR_DE_euler_",body.part,".pdf"))
...
plot(euler(DE_joined$euler[, c(3,6,9)], shape = "ellipse"),
     quantities = list(fontsize =16),
     fill = color_transparent2,
     lty = 1, labels =list(labels= c("S", "F", "SxF"), font = 2, fontsize = 18))
...
{pdf(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Clustering/edgeR_DE_MDSbySeasonStress_",body.part,".pdf"),
     height = 5, 
     width = 5)  
...
{pdf(file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Clustering/edgeR_DE_MDSbySeasonStress_dims1to5_",body.part,".pdf"),
     height = 5.5, 
     width = 5)  
...
write_tsv(tmmPCA.data.frame.genes,paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_dataframes/edgeR_DE_tmmPCA_", body.part,".tsv"))
...
ggsave(paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Clustering/edgeR_DE_prcomp_propVariance_",body.part,".pdf"), height = 4.5, width = 6) 
...
ggsave(plot = pca.plot, paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/Clustering/edgeR_DE_prcomp_",PC_columns[1], PC_columns[2],"_",body.part,".pdf"), height = 4.5, width = 4) 
...
ggsave(plot = ngenes, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/edgeR_DE_NExonsSimesPlots_", body.part,".pdf"), height = 4, width = 12) 
volc <- grid.arrange(volc1, volc2, volc3, nrow = 1)
ggsave(plot = volc, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/edgeR_DE_volcanoPlots_", body.part,".pdf"), height = 4, width = 12) 
bp <- grid.arrange( bp1, bp2, bp3, bp4, nrow = 2, ncol =2)
ggsave(plot = bp, file = paste0("02_edgeR_genes_SxF/02_edgeR_genes/DE_figures/edgeR_DE_SandF_foldchange_", body.part,".pdf"), height = 8, width = 8) 
