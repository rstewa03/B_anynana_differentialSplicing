#' ---
#' title: "REVIGO analyses for SxF DS and DE genesets"
...
setwd("~/OneDrive/B_anynana_altSplicing/PanicTime/genelists/REVIGO_I/treemap/")
...
setwd("/Users/rachelsteward/OneDrive/B_anynana_altSplicing/PanicTime/genelists/REVIGO_I/SemSpace/")
...
ab_BP <- plot_grid(plots[[1]]+ theme(legend.position = "none"),
                   ...
                   NULL,
                   cowplot::get_legend(plots[[3]]),
                   NULL,
                   ncol = 2, nrow =2, labels = c("A", "B", ""), 
                   rel_widths = c(1,1), rel_heights = c(1,.1))
ggsave(ab_BP, filename = "ab_BP_parchild_REVIGO_semspace.pdf", height = 7, width = 13)

th_BP <- plot_grid(plots[[5]]+ theme(legend.position = "none"), #DE 
                   plots[[7]]+ theme(legend.position = "none"), #DEDS
                   NULL,
                   cowplot::get_legend(plots[[3]]),
                   NULL,
                   ncol = 2, nrow =2, labels = c("A", "B", ""), 
...
th_MF <- plot_grid(plots[[6]]+ theme(legend.position = "none"),
...
semspace.colors2 <- c("#7876B1","#BC3C29")
# 
...
ab_BP_overlap <- all_terms[, c(1,2,4)]%>% filter_at(vars(-term_ID), any_vars(. > 0)) %>% mutate(sum = rowSums(.[2:3]), group = "ab_BP") %>% filter(sum >1)
colnames(ab_BP_overlap) <- c("termID", "DE", "DS", "sum", "group")
ab_MF_overlap <- all_terms[, c(1,3,5)] %>% filter_at(vars(-term_ID), any_vars(. > 0))%>% mutate(sum = rowSums(.[2:3]), group = "ab_MF") %>% filter(sum >1)
colnames(ab_MF_overlap) <- c("termID", "DE", "DS", "sum", "group")
th_BP_overlap <- all_terms[, c(1,6,8)]%>% filter_at(vars(-term_ID), any_vars(. > 0))%>% mutate(sum = rowSums(.[2:3]), group = "th_BP") %>% filter(sum >1) 
colnames(th_BP_overlap) <- c("termID", "DE", "DS", "sum", "group")
th_MF_overlap <- all_terms[, c(1,7,9)]%>% filter_at(vars(-term_ID), any_vars(. > 0))%>% mutate(sum = rowSums(.[2:3]), group = "th_MF") %>% filter(sum >1) 
colnames(th_MF_overlap) <- c("termID", "DE",  "DS", "sum", "group")
# 
# overlap <- union(ab_BP_overlap, ab_MF_overlap) %>% union(th_BP_overlap)%>% union(th_MF_overlap)
# write.csv(overlap, "/Users/rachelsteward/OneDrive/B_anynana_altSplicing/PanicTime/genelists/REVIGO_I/overlappping_go_terms.csv")

setwd("/Users/rachelsteward/OneDrive/B_anynana_altSplicing/PanicTime/genelists/REVIGO_I/SemSpace/all/")
...
for (i in 1:length(my_combined)) {
  my_combined[[i]] <- my_combined[[i]][,c(1,4,5)] %>% 
    mutate(plot_X = as.numeric(PlotX), 
           plot_Y = as.numeric(PlotY)) %>% 
    select(TermID, plot_X, plot_Y) %>% rename(term_ID = TermID)
}

my_semspace_ab_BP <- my_semspace[grepl("ab_DE_I_BP|ab_DEDS_I_BP|ab_DS_I_BP", names(my_semspace))]
my_semspace_th_BP <- my_semspace[grepl("th_DE_I_BP|th_DEDS_I_BP|th_DS_I_BP", names(my_semspace))] 
my_semspace_ab_MF <- my_semspace[grepl("ab_DE_I_MF|ab_DEDS_I_MF|ab_DS_I_MF", names(my_semspace))]
my_semspace_th_MF <- my_semspace[grepl("th_DE_I_MF|th_DEDS_I_MF|th_DS_I_MF", names(my_semspace))] 

combined_list <- list()
factorNum <- c("DE_only", "DS_only")
for (i in 1:length(my_semspace_ab_BP)) {
  my_semspace_ab_BP[[i]] <- my_semspace_ab_BP[[i]] %>% dplyr::select(-c(plot_X, plot_Y)) %>% filter(dispensability < 0.3) %>% left_join(my_combined$ab_I_BP) %>% mutate(description = as.character(description))
...
for (i in 1:length(my_semspace_th_BP)) {
  my_semspace_th_BP[[i]] <- my_semspace_th_BP[[i]] %>% dplyr::select(-c(plot_X, plot_Y)) %>% filter(dispensability < 0.3) %>% left_join(my_combined$th_I_BP)%>% mutate(description = as.character(description))
...
for (i in 1:length(my_semspace_ab_MF)) {
  my_semspace_ab_MF[[i]] <- my_semspace_ab_MF[[i]] %>% dplyr::select(-c(plot_X, plot_Y)) %>%filter(dispensability < 0.3)  %>% left_join(my_combined$ab_I_MF) %>% mutate(description = as.character(description))
...
for (i in 1:length(my_semspace_th_MF)) {
  my_semspace_th_MF[[i]] <- my_semspace_th_MF[[i]] %>% dplyr::select(-c(plot_X, plot_Y)) %>%filter(dispensability < 0.3)  %>% left_join(my_combined$th_I_MF)%>% mutate(description = as.character(description))
...
semspace.colors2 <- c("#0072B5","#BC3C29")
semspace.colors3 <-  c("dodgerblue4","darkred")
...
for (i in 1:length(combined_list)){
  ...
  plots2[[i]] <- local({
    p1 <- ggplot( ) +
      ...
      scale_color_manual(values = semspace.colors2[c(2,1)],
                         labels = c("DS only","DE only"))+
      scale_size( range=c(5, 20), guide = FALSE) + 
      ...
      geom_text_repel(
        ...
        direction     = "y",
        hjust         = 1) +
      scale_color_manual(values = semspace.colors3[c(2,1)],
                         labels = c("DS only","DE only"))+
...
for (i in 1:length(combined_list)){
  ...
  plots4[[i]] <- local({
    p1 <- ggplot( ) +
      geom_point(data = combined_list[[i]], aes( plot_X, plot_Y, size = plot_size, colour = name ), alpha =0.75) + 
      geom_point( data = combined_list[[i]], aes(plot_X, plot_Y, size = plot_size), shape = 21, fill = "transparent", colour = I (alpha ("black", 0.6) )) +
      scale_color_manual(values = semspace.colors2[c(1,2)],
...
i = 3
save(plots4, file = paste0("Fig2_parchild_REVIGO_semspacePlots_interaction.Rdata"))
save(plots5, file = paste0("Fig2_0.01_parchild_REVIGO_semspacePlots_interaction.Rdata"))


scale_color_manual(values = semspace.colors2[c(2)],
                   labels = c("DS only"))